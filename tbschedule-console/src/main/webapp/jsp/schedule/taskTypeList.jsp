...
%>
     <tr onclick="openDetail(this,'<%=taskTypes.get(i).getBaseTaskType()%>')">
     	...
     	<td width="120" align="center">
     		<a target="taskDetail" href="taskTypeEdit?taskType=<%=taskTypes.get(i).getBaseTaskType()%>"  style="color:#0000CD">编辑</a>
     		<a target="taskDetail" href="taskTypeDeal?action=clearTaskType&taskType=<%=taskTypes.get(i).getBaseTaskType()%>"  style="color:#0000CD">清理</a>
...
<%
	if ("true".equals(isManager)) {
%>
<a target="taskDetail" href="taskTypeEdit?taskType=-1"  style="color:#0000CD">创建新任务...</a>
<%
	}
%>
运行期信息：<br/>
...
function openDetail(obj,baseTaskType){
	...
	obj.bgColor="#FFD700";
	oldSelectRow = obj;
	document.getElementById("showTaskDetail").src = "taskTypeInfo?baseTaskType=" + baseTaskType;
}
...
function validateDel(str) {
    ...
    if(flag) {
        window.location.href="taskTypeDeal?action=deleteTaskType&taskType="+str; 
    }
}
</script>
